...
const SYNC_CLIENT_ID = import.meta.env?.VITE_AAD_CLIENT_ID || '';
const SYNC_TENANT_ID = import.meta.env?.VITE_AAD_TENANT_ID || '';
const SYNC_ALLOWED_DOMAIN = import.meta.env?.VITE_AAD_ALLOWED_DOMAIN || '';
const SYNC_API_SCOPE = import.meta.env?.VITE_AAD_API_SCOPE || '';
...
export function getAuthConfigErrors(cfg) {
    const errs = [];
    ...
    if (!clientId)
        errs.push("Missing MSAL_CLIENT_ID");
    if (!tenantId)
        errs.push("Missing MSAL_TENANT_ID");
    if (!allowedDomain)
        errs.push("Missing MSAL_ALLOWED_DOMAIN");
    if (!apiScope)
        errs.push("Missing MSAL_API_SCOPE (required for calling your backend API)");
    return errs;
}
// Export synchronous helpers so existing components can still import them at build time.
export const loginRequest = { scopes: [SYNC_API_SCOPE, 'openid', 'profile', 'email'].filter(Boolean) };
