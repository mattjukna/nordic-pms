...
export const TrendsTab: React.FC = () => {
  ...
  const { filteredIntake, filteredOutput, filteredDispatch, dateLabel, rangeStartTs, rangeEndTs } = useMemo(() => {
    ...
    if (customStart) {
      ...
      switch (timeRange) {
        case "day": startTs = todayStart; label = t('trends.todayLabel'); break;
        case "week": startTs = sub(7); label = t('trends.last7Days'); break;
        case "month": startTs = sub(30); label = t('trends.last30Days'); break;
        case "quarter": startTs = sub(90); label = t('trends.last90Days'); break;
        case "year": startTs = sub(365); label = t('trends.last365Days'); break;
        case "all": startTs = 0; label = t('trends.allTime'); break;
      }
    }
  ...
  const costBreakdownData = useMemo(() => {
    return [
      { name: t('trends.milkCost'), value: financialKPIs.rawMaterialCost, color: '#3b82f6' },
      { name: t('trends.processing'), value: financialKPIs.processingCost, color: '#f59e0b' },
      { name: t('trends.discarded'), value: financialKPIs.discardedLoss, color: '#ef4444' },
    ].filter(i => i.value > 0);
  }, [financialKPIs]);
  ...
  const productMixData = useMemo(() => {
    ...
    top.push({ id: 'other', name: t('trends.other'), value: sorted.slice(6).reduce((s, it) => s + it.value, 0) });
    return top;
      ...
      <GlassCard className="p-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-2 text-slate-700 min-w-0">
            ...
            <div className="text-sm font-semibold">{t('trends.analyticsRange')}</div>
            <div className="text-sm text-slate-500">{dateLabel}</div>
            {timeRange !== 'all' && <span className="text-[10px] text-slate-400 hidden md:inline">{t('trends.vsPrevious', { period: timeRange === 'day' ? t('trends.day') : timeRange === 'week' ? t('trends.days7') : timeRange === 'month' ? t('trends.days30') : timeRange === 'quarter' ? t('trends.days90') : t('trends.days365') })}</span>}
          </div>
        ...
        {activeView === 'production' && (
          <div className="mt-4 flex flex-wrap gap-2">
            <button onClick={() => setProdSubtab('output')} className={`flex items-center gap-2 px-3 py-1.5 text-xs font-bold rounded-md transition-all duration-200 uppercase whitespace-nowrap ${prodSubtab === 'output' ? 'bg-white text-blue-700 shadow-sm border border-slate-200' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100'}`}>
              <Factory className="h-4 w-4" /> {t('trends.output')}
            </button>
            <button onClick={() => setProdSubtab('intake')} className={`flex items-center gap-2 px-3 py-1.5 text-xs font-bold rounded-md transition-all duration-200 uppercase whitespace-nowrap ${prodSubtab === 'intake' ? 'bg-white text-blue-700 shadow-sm border border-slate-200' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100'}`}>
              <Droplets className="h-4 w-4" /> {t('trends.intake')}
            </button>
          </div>
        )}
      </GlassCard>
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <GlassCard className="p-4 hover:shadow-md transition-shadow duration-200" hint="Total revenue from dispatches with shipped or delivered status in the selected time period. The delta badge shows percentage change compared to the previous period of equal length.">
              <div className="flex items-center justify-between">
                <div className="text-xs font-bold uppercase text-slate-500">{t('trends.revenue')}</div>
              ...
              <div className="mt-1 text-xs text-slate-500">{t('trends.fromDispatches', { count: String(filteredDispatch.length) })}</div>
            </GlassCard>
            ...
            <GlassCard className="p-4 hover:shadow-md transition-shadow duration-200" hint="Total cost of raw milk purchases, calculated as quantity (kg) × agreed price per kg for each intake. Delta badge shows change vs previous period — green means costs decreased.">
              <div className="flex items-center justify-between">
                <div className="text-xs font-bold uppercase text-slate-500">{t('trends.milkSpend')}</div>
              ...
              <div className="mt-1 text-xs text-slate-500">{t('trends.avgPrice', { price: financialKPIs.avgMilkPrice.toFixed(3) })}</div>
            </GlassCard>
            ...
            <GlassCard className="p-4 hover:shadow-md transition-shadow duration-200" hint="Cost of Goods Sold = raw milk cost + estimated processing cost + value of discarded milk. Represents the total cost attributable to production.">
              <div className="flex items-center justify-between">
                <div className="text-xs font-bold uppercase text-slate-500">{t('trends.totalCogs')}</div>
              </div>
              <div className="mt-2 text-xl md:text-2xl font-extrabold text-slate-800">{formatEur(financialKPIs.cogs)}</div>
              <div className="mt-1 text-xs text-slate-500">{t('trends.cogsDesc')}</div>
            </GlassCard>
            ...
            <GlassCard className="p-4 hover:shadow-md transition-shadow duration-200" hint="Gross margin = (Revenue − COGS) ÷ Revenue × 100. Shows how much of each euro earned is retained as profit before operating expenses.">
              <div className="flex items-center justify-between">
                <div className="text-xs font-bold uppercase text-slate-500">{t('trends.grossMargin')}</div>
              ...
              <div className="mt-1 text-xs text-slate-500">{formatEur(financialKPIs.margin)} • {t('trends.avgSellPrice', { price: financialKPIs.avgSalesPrice.toFixed(2) })}</div>
            </GlassCard>
          </div>
          ...
          <GlassCard className="p-4" hint="Daily comparison of dispatch revenue (blue bars) vs milk purchase cost (amber bars). The green profit line shows Revenue − Cost per day. Helps identify profitable and unprofitable periods.">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm font-extrabold text-slate-800">{t('trends.revenuVsCosts')}</div>
              </div>
              <div className="text-xs text-slate-500 hidden sm:block">
                {t('trends.profit')} {formatEur(financialKPIs.margin)} • {financialKPIs.marginPct.toFixed(1)}%
              </div>
            </div>
            {dailyFinancialData.length === 0 ? <EmptyState title={t('trends.noFinancialData')} hint={t('trends.tryWiderRange')} /> : (
              <div className="h-72 md:h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={dailyFinancialData} margin={{ left: 0, right: 16, top: 8, bottom: 8 }}>
                    ...
                    <Bar dataKey="revenue" name={t('trends.revenue')} fill="url(#finGradRevenue)" barSize={24} animationDuration={800} animationEasing="ease-out" radius={[3, 3, 0, 0]} />
                    <Bar dataKey="cost" name={t('trends.milkCost')} fill="url(#finGradCost)" barSize={18} animationDuration={800} animationEasing="ease-out" radius={[3, 3, 0, 0]} />
                    <Line type="monotone" dataKey="profit" name={t('trends.profit')} stroke="#10b981" strokeWidth={2.5} dot={false} animationDuration={1000} animationEasing="ease-out" />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
            )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <GlassCard className="p-4" hint="Horizontal bar chart ranking buyers by total revenue from shipped dispatches. Useful for identifying revenue concentration — ideally no single buyer should dominate.">
              <div className="flex items-center gap-2 mb-3">
                ...
                <div className="text-sm font-extrabold text-slate-800">{t('trends.revenueByBuyer')}</div>
              </div>
              {buyerRevenueData.length === 0 ? <EmptyState title={t('trends.noBuyerData')} /> : (
                <div style={{ height: Math.max(200, buyerRevenueData.length * 36 + 40) }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={buyerRevenueData} layout="vertical" margin={{ left: 16, right: 16 }}>
                      ...
                      <Bar dataKey="value" name={t('trends.revenue')} fill="#8b5cf6" barSize={18} animationDuration={800} animationEasing="ease-out" radius={[0, 4, 4, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              )}
            ...
            <GlassCard className="p-4" hint="Pie chart showing the split of total costs: raw milk purchases, estimated processing costs, and losses from discarded milk.">
              <div className="flex items-center gap-2 mb-3">
                ...
                <div className="text-sm font-extrabold text-slate-800">{t('trends.costBreakdown')}</div>
              </div>
              {costBreakdownData.length === 0 ? <EmptyState title={t('trends.noCostData')} /> : (
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
          ...
          <GlassCard className="p-4" hint="Table of all products ranked by revenue. Shows total revenue, shipped volume (kg), average selling price per kg, and each product's share of total revenue.">
            <div className="flex items-center gap-2 mb-3">
              ...
              <div className="text-sm font-extrabold text-slate-800">{t('trends.revenueByProduct')}</div>
            </div>
            {productRevenueData.length === 0 ? <EmptyState title={t('trends.noProductRevenueData')} /> : (
              <div className="mt-3 w-full overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-slate-50 text-slate-500 text-xs uppercase font-bold sticky top-0 z-10">
                    <tr>
                      <th className="p-2">{t('trends.thProduct')}</th>
                      <th className="p-2 text-right">{t('trends.thRevenue')}</th>
                      <th className="p-2 text-right">{t('trends.thShippedKg')}</th>
                      <th className="p-2 text-right">{t('trends.thAvgEurKg')}</th>
                      <th className="p-2 hidden md:table-cell">{t('trends.thShare')}</th>
                    </tr>
                  </thead>
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <GlassCard className="p-4 hover:shadow-md transition-shadow duration-200" hint="Net milk intake = total intake minus discarded loads. Counts only milk accepted for production in the selected period.">
              <div className="flex items-center justify-between">
                <div className="text-xs font-bold uppercase text-slate-500">{t('trends.intakeNet')}</div>
              ...
              <div className="mt-1 text-xs text-slate-500">{t('trends.topPrefix', { name: productionKPIs.topProductName })}</div>
            </GlassCard>
            ...
            <GlassCard className="p-4 hover:shadow-md transition-shadow duration-200" hint="Total weight of finished products produced. Variance = Output − (Net Intake × target yield factor).">
              <div className="flex items-center justify-between">
                <div className="text-xs font-bold uppercase text-slate-500">{t('trends.output')}</div>
              ...
              <div className="mt-1 text-xs text-slate-500">{t('trends.variancePrefix', { value: formatKg(productionKPIs.varianceKg) })}</div>
            </GlassCard>
            ...
            <GlassCard className="p-4 hover:shadow-md transition-shadow duration-200" hint="Production yield = Total Output ÷ Net Intake × 100. Target is 12.5% (typical for cheese production). Higher yield means more efficient conversion of milk to product.">
              <div className="flex items-center justify-between">
                <div className="text-xs font-bold uppercase text-slate-500">{t('trends.yield')}</div>
              ...
              <div className="mt-1 text-xs text-slate-500">{t('trends.targetPrefix', { value: `${(TARGET_YIELD_FACTOR * 100).toFixed(1)}%` })}</div>
            </GlassCard>
            ...
            <GlassCard className="p-4 hover:shadow-md transition-shadow duration-200" hint="Milk loads rejected during intake due to quality issues (temperature, pH, contamination). Shows total discarded kg and percentage of total intake.">
              <div className="text-xs font-bold uppercase text-slate-500">{t('trends.discarded')}</div>
              <div className="mt-2 text-xl md:text-2xl font-extrabold text-red-600">{formatKg(productionKPIs.discardedKg)}</div>
              <div className="mt-1 text-xs text-slate-500">{productionKPIs.discardedPct.toFixed(1)}{t('trends.pctOfTotalIntake')}</div>
            </GlassCard>
          </div>
          ...
          {prodSubtab === 'output' && (
            <>
              <GlassCard className="p-4 flex items-center justify-between gap-4 flex-wrap" hint="Use the product filter to drill down into a specific product's production, shipment, and revenue data.">
                <div>
                  <div className="text-sm font-extrabold text-slate-800">{t('trends.outputOverview')}</div>
                  <div className="text-xs text-slate-500 mt-1">{t('trends.selectProductInspect')}</div>
                </div>
                <div className="flex gap-2 flex-wrap items-center min-w-0">
                  <div className="text-xs text-slate-500 uppercase font-bold">{t('trends.thProduct')}</div>
                  <select value={productFilter} onChange={e => setProductFilter(e.target.value)} className="bg-white border border-slate-200 rounded p-2 text-sm min-w-0 w-full md:w-auto">
                    <option value={ALL}>{t('trends.allProducts')}</option>
              ...
              {productFilter === ALL ? (
                <GlassCard className="p-4" hint="Summary table showing each product's produced quantity, shipped quantity, revenue, average price per kg, and net stock (produced − shipped).">
                  <div className="text-sm font-extrabold text-slate-800 mb-3">{t('trends.productSummary')}</div>
                  <div className="w-full overflow-x-auto">
                    <table className="w-full text-left text-sm">
                      <thead className="bg-slate-50 text-slate-500 text-xs uppercase font-bold sticky top-0 z-10">
                        <tr>
                          <th className="p-2">{t('trends.thProduct')}</th>
                          <th className="p-2 text-right">{t('trends.thProduced')}</th>
                          <th className="p-2 text-right">{t('trends.thShipped')}</th>
                          <th className="p-2 text-right hidden md:table-cell">{t('trends.thRevenue')}</th>
                          <th className="p-2 text-right hidden md:table-cell">{t('trends.thAvgEurKg')}</th>
                          <th className="p-2 text-right">{t('trends.thNet')}</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {perProductRows.length === 0 ? (
                          <tr><td colSpan={6} className="p-6 text-center text-slate-400 italic">{t('trends.noProductActivity')}</td></tr>
                ...
                <>
                  <GlassCard className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4" hint="Key metrics for the selected product: total produced weight, total shipped weight, and total revenue from shipped dispatches.">
                    <div className="p-2">
                      <div className="text-xs font-bold uppercase text-slate-500">{t('trends.produced')}</div>
                    ...
                    <div className="p-2">
                      <div className="text-xs font-bold uppercase text-slate-500">{t('trends.shipped')}</div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                    <GlassCard className="p-4 lg:col-span-2" hint="Area chart showing daily production output (blue) vs daily shipped quantity (green) for the selected product. Gaps indicate stock building up or being depleted.">
                      <div className="text-sm font-extrabold text-slate-800 mb-3">{t('trends.dailyProducedVsShipped')}</div>
                      {buildDailySeriesForProduct(outputForProduct, dispatchForProduct).length === 0 ? <EmptyState title={t('trends.noDailyData')} /> : (
                        <div className="h-60 md:h-72">
                          <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart data={buildDailySeriesForProduct(outputForProduct, dispatchForProduct)}>
                              ...
                              <Area type="monotone" dataKey="produced" name={t('trends.produced')} stroke="#3b82f6" fill="url(#prodGradProd)" strokeWidth={2} animationDuration={800} />
                              <Line type="monotone" dataKey="shipped" name={t('trends.shipped')} stroke="#10b981" strokeWidth={2} dot={false} animationDuration={800} />
                            </ComposedChart>
                          </ResponsiveContainer>
                        </div>
                      )}
                    ...
                    <GlassCard className="p-4 lg:col-span-1" hint="Bar chart showing daily revenue from shipped dispatches for the selected product.">
                      <div className="text-sm font-extrabold text-slate-800 mb-3">{t('trends.dailyRevenue')}</div>
                      {buildDailySeriesForProduct(outputForProduct, dispatchForProduct).length === 0 ? <EmptyState title={t('trends.noRevenueData')} /> : (
                        <div className="h-60 md:h-72">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={buildDailySeriesForProduct(outputForProduct, dispatchForProduct)}>
                              ...
                              <Bar dataKey="revenue" name={t('trends.revenue')} fill="url(#prodGradRev)" barSize={20} animationDuration={800} radius={[3, 3, 0, 0]} />
                            </BarChart>
                          </ResponsiveContainer>
                        </div>
                      )}
              ...
              <GlassCard className="p-4" hint="Pie chart showing the share of each product in total output by weight. Helps visualize production focus and diversification.">
                <div className="text-sm font-extrabold text-slate-800 mb-3">{t('trends.productMixOutput')}</div>
                {productMixData.length === 0 ? <EmptyState title={t('trends.noOutputData')} /> : (
              ...
              {dailyProductionData.length > 1 && (
                <GlassCard className="p-4" hint="Daily yield percentage (output ÷ intake × 100) over time. The red dashed line shows the 12.5% target. Consistent yield above target indicates efficient production.">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div className="text-sm font-extrabold text-slate-800">{t('trends.dailyYieldTrend')}</div>
                    <span className="text-xs text-slate-400">{t('trends.targetYield', { value: `${(TARGET_YIELD_FACTOR * 100).toFixed(1)}%` })}</span>
                  </div>
                  <div className="h-56 md:h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={dailyProductionData} margin={{ left: 0, right: 16, top: 8, bottom: 8 }}>
                        ...
                        <Area type="monotone" dataKey="yield" name={t('trends.yieldPct')} stroke="#10b981" fill="url(#yieldGrad)" strokeWidth={2} dot={false} animationDuration={800} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </GlassCard>
          ...
          {prodSubtab === 'intake' && (
            <>
              <GlassCard className="p-4 flex items-center justify-between gap-4 flex-wrap" hint="Intake analytics filtered by milk type and supplier. Use the filters to drill down into specific intake segments.">
                <div>
                  <div className="text-sm font-extrabold text-slate-800">{t('trends.intakeOverview')}</div>
                  <div className="text-xs text-slate-500 mt-1">{t('trends.filtersApply')}</div>
                </div>
                <div className="flex gap-2 flex-wrap items-center min-w-0">
                  <select value={milkTypeFilter} onChange={e => setMilkTypeFilter(e.target.value)} className="bg-white border border-slate-200 rounded p-2 text-sm min-w-0 w-full md:w-auto">
                    <option value="__all__">{t('trends.allMilkTypes')}</option>
                  ...
                  <select value={supplierFilter} onChange={e => setSupplierFilter(e.target.value)} className="bg-white border border-slate-200 rounded p-2 text-sm min-w-0 w-full md:w-auto">
                    <option value="__all__">{t('trends.allSuppliers')}</option>
              ...
              <GlassCard className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4" hint="Net Intake: accepted milk volume. Discarded: rejected milk volume. Avg Fat/Prot/pH: average quality parameters for accepted milk loads in this period.">
                <div>
                  <div className="text-xs font-bold uppercase text-slate-500">{t('trends.netIntake')}</div>
                ...
                <div>
                  <div className="text-xs font-bold uppercase text-slate-500">{t('trends.avgFatProtPh')}</div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                <GlassCard className="p-4 lg:col-span-2" hint="Area chart showing daily accepted intake (blue) vs discarded volume (red). Spikes in discards may indicate supply chain quality issues.">
                  <div className="text-sm font-extrabold text-slate-800 mb-3">{t('trends.dailyIntakeVsDiscarded')}</div>
                  {dailyIntakeData.length === 0 ? <EmptyState title={t('trends.noIntakeData')} /> : (
                    <div className="h-64 md:h-72">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={dailyIntakeData}>
                          ...
                          <Legend wrapperStyle={{ fontSize: 12 }} />
                          <Area type="monotone" dataKey="intake" name={t('trends.intake')} stroke="#3b82f6" fill="url(#intGradIntake)" strokeWidth={2} animationDuration={800} />
                          <Area type="monotone" dataKey="discarded" name={t('trends.discarded')} stroke="#ef4444" fill="url(#intGradDiscard)" strokeWidth={2} animationDuration={800} />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
                  )}
                ...
                <GlassCard className="p-4 lg:col-span-1" hint="Pie chart showing the volume share of each milk type (e.g. cow, goat, sheep) in total intake.">
                  <div className="text-sm font-extrabold text-slate-800 mb-3">{t('trends.milkTypes')}</div>
                  {milkTypeMixFiltered.length === 0 ? <EmptyState title={t('trends.noMilkTypeData')} /> : (
                    <div className="h-56 md:h-72">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
              ...
              <GlassCard className="p-4" hint="Bar chart showing each supplier's contribution to total intake volume (kg). Helps identify supplier concentration and diversification.">
                <div className="text-sm font-extrabold text-slate-800 mb-3">{t('trends.supplierMix')}</div>
                {supplierMix.length === 0 ? <EmptyState title={t('trends.noSupplierData')} /> : (
                  <div className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={supplierMix}>
                        ...
                        <YAxis tick={{ fill: '#64748b', fontSize: 11 }} />
                        <Tooltip content={<ChartTooltip kgKeys={['value']} />} />
                        <Bar dataKey="value" name={t('trends.kg')} fill="url(#supGradBar)" barSize={28} animationDuration={800} radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                )}
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <GlassCard className="p-4 hover:shadow-md transition-shadow duration-200" hint="Percentage of intake loads meeting all quality standards: temperature ≤8°C and pH within 6.55–6.74 range. Green ≥90%, amber ≥70%, red <70%.">
              <div className="flex items-center justify-between">
                <div className="text-xs font-bold uppercase text-slate-500">{t('trends.compliance')}</div>
              ...
              <div className="mt-1 text-xs text-slate-500">{t('trends.loadsAnalyzed', { count: String(qualityKPIs.total) })}</div>
            </GlassCard>
            ...
            <GlassCard className="p-4 hover:shadow-md transition-shadow duration-200" hint="Average fat percentage across all intake loads in the period. Target is 4.0%. Higher fat content generally means better cheese yield.">
              <div className="text-xs font-bold uppercase text-slate-500">{t('trends.avgFat')}</div>
              <div className="mt-2 text-xl md:text-2xl font-extrabold text-slate-800">{qualityKPIs.avgFat.toFixed(2)}%</div>
              <div className="mt-1 text-xs text-slate-500">{t('trends.targetPrefix', { value: `${FAT_TARGET.toFixed(1)}%` })}</div>
            </GlassCard>
            ...
            <GlassCard className="p-4 hover:shadow-md transition-shadow duration-200" hint="Average protein percentage across all intake loads. Target is 3.2%. Protein is essential for cheese curd formation.">
              <div className="text-xs font-bold uppercase text-slate-500">{t('trends.avgProtein')}</div>
              <div className="mt-2 text-xl md:text-2xl font-extrabold text-slate-800">{qualityKPIs.avgProt.toFixed(2)}%</div>
              <div className="mt-1 text-xs text-slate-500">{t('trends.targetPrefix', { value: `${PROT_TARGET.toFixed(1)}%` })}</div>
            </GlassCard>
            ...
            <GlassCard className="p-4 hover:shadow-md transition-shadow duration-200" hint="Average pH across all intake loads. Standard range is 6.55–6.74. pH outside this range may indicate spoilage or contamination.">
              <div className="text-xs font-bold uppercase text-slate-500">{t('trends.avgPh')}</div>
              <div className="mt-2 text-xl md:text-2xl font-extrabold text-slate-800">{qualityKPIs.avgPh.toFixed(2)}</div>
              <div className="mt-1 text-xs text-slate-500">{t('trends.stdRange', { range: `${PH_MIN}–${PH_MAX}` })}</div>
            </GlassCard>
          </div>
          ...
          {(qualityKPIs.highTempCount + qualityKPIs.badPhCount > 0) && (
            <GlassCard className="p-4 bg-red-50/60 border border-red-200" hint="Count of temperature violations (>8°C) and pH violations (outside 6.55–6.74) detected in intake loads during the current period.">
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm font-extrabold text-red-800">{t('trends.qualityViolations')}</div>
              </div>
              <div className="mt-2 flex gap-6 text-sm text-red-700">
                <span>{t('trends.tempAbove', { max: String(TEMP_MAX) })} <strong>{qualityKPIs.highTempCount}</strong></span>
                <span>{t('trends.phOutOfRange')} <strong>{qualityKPIs.badPhCount}</strong></span>
              </div>
            </GlassCard>
          )}
          ...
          <GlassCard className="p-4" hint="Line chart tracking daily average fat % (blue), protein % (green), and pH (red) over time. Dashed reference lines show target values. Helps identify quality trends and seasonal patterns.">
            <div className="text-sm font-extrabold text-slate-800 mb-3">{t('trends.qualityTrends')}</div>
            {qualityTrendData.length === 0 ? <EmptyState title={t('trends.noQualityData')} /> : (
              <div className="h-72 md:h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={qualityTrendData} margin={{ left: 0, right: 16, top: 8, bottom: 8 }}>
                    ...
                    <Line yAxisId="left" type="monotone" dataKey="fat" name={t('trends.fatPct')} stroke="#3b82f6" strokeWidth={2} dot={false} animationDuration={800} />
                    <Line yAxisId="left" type="monotone" dataKey="protein" name={t('trends.proteinPct')} stroke="#10b981" strokeWidth={2} dot={false} animationDuration={800} />
                    <Line yAxisId="right" type="monotone" dataKey="ph" name={t('trends.ph')} stroke="#ef4444" strokeWidth={2} dot={false} animationDuration={800} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
          ...
          <GlassCard className="p-4" hint="Table ranking suppliers by quality compliance. Shows number of loads, total volume, average fat/protein/pH values, and overall compliance percentage. Use to identify unreliable suppliers.">
            <div className="flex items-center gap-2 mb-3">
              ...
              <div className="text-sm font-extrabold text-slate-800">{t('trends.supplierQualityScorecard')}</div>
            </div>
            {supplierScorecard.length === 0 ? <EmptyState title={t('trends.noSupplierQualityData')} /> : (
              <div className="w-full overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-slate-50 text-slate-500 text-xs uppercase font-bold sticky top-0 z-10">
                    <tr>
                      <th className="p-2">{t('trends.thSupplier')}</th>
                      <th className="p-2 text-right">{t('trends.thLoads')}</th>
                      <th className="p-2 text-right">{t('trends.thVolume')}</th>
                      <th className="p-2 text-right hidden md:table-cell">{t('trends.thAvgFat')}</th>
                      <th className="p-2 text-right hidden md:table-cell">{t('trends.thAvgProt')}</th>
                      <th className="p-2 text-right hidden md:table-cell">{t('trends.thAvgPh')}</th>
                      <th className="p-2 text-right">{t('trends.thCompliance')}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
          ...
          <GlassCard className="p-4" hint="Table of specific intake loads with quality parameter violations. Red-highlighted values indicate fat, protein, pH, or temperature outside acceptable ranges. Review with suppliers.">
            <div className="text-sm font-extrabold text-slate-800 mb-3">{t('trends.topQualityIssues')}</div>
            {outliers.length === 0 ? (
              <div className="text-slate-400 italic text-sm">{t('trends.noIssuesInRange')}</div>
            ) : (
              <div className="w-full overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-slate-50 text-slate-500 text-xs uppercase font-bold sticky top-0 z-10">
                    <tr>
                      <th className="p-2">{t('trends.thDate')}</th>
                      <th className="p-2">{t('trends.thSupplier')}</th>
                      <th className="p-2">{t('trends.thType')}</th>
                      <th className="p-2 text-right">{t('trends.thKg')}</th>
                      <th className="p-2 text-right">{t('trends.thFat')}</th>
                      <th className="p-2 text-right">{t('trends.thProt')}</th>
                      <th className="p-2 text-right">{t('trends.ph')}</th>
                      <th className="p-2 text-right">{t('trends.thTemp')}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {outliers.map(o => (
